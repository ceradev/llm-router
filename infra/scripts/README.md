...
```powershell
powershell -ExecutionPolicy Bypass -File "infra/scripts/monitor_catalog.ps1"
```
...
```powershell
powershell -ExecutionPolicy Bypass -File "infra/scripts/run_catalog_eval.ps1"
```
...
```powershell
powershell -ExecutionPolicy Bypass -File "infra/scripts/run_catalog_eval.ps1" -MaxModels 5 -MaxLive 2
```

### 3. Correr solo heuristico

```powershell
powershell -ExecutionPolicy Bypass -File "infra/scripts/run_catalog_eval.ps1" -MaxModels 20 -MaxLive 0
```

### 4. Correr solo live sobre provisionales

```powershell
powershell -ExecutionPolicy Bypass -File "infra/scripts/run_catalog_eval.ps1" -MaxModels 0 -MaxLive 10
```

### 5. Limitar a ciertos providers

```powershell
powershell -ExecutionPolicy Bypass -File "infra/scripts/run_catalog_eval.ps1" -ProviderAllowlist "openai,anthropic"
```

### 6. Re-evaluar tambien modelos ya verificados

```powershell
powershell -ExecutionPolicy Bypass -File "infra/scripts/run_catalog_eval.ps1" -IncludeVerifiedLive
```
