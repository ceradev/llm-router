...
const ADMIN_KEY_STORAGE = "llm_router_admin_key"
...
export function getStoredAdminKey(): string {
  if (!isBrowser()) return ""
  try {
    return globalThis.window.sessionStorage.getItem(ADMIN_KEY_STORAGE) ?? ""
  } catch {
    return ""
  }
}
...
export function setStoredAdminKey(key: string): void {
  if (!isBrowser()) return
  try {
    globalThis.window.sessionStorage.setItem(ADMIN_KEY_STORAGE, key)
  } catch {
    // ignore
  }
}
...
export function clearStoredAdminKey(): void {
  if (!isBrowser()) return
  try {
    globalThis.window.sessionStorage.removeItem(ADMIN_KEY_STORAGE)
  } catch {
    // ignore
  }
}
...
/**
 * Verifies the admin key against the backend without persisting it.
 * Use this before saving to sessionStorage.
 */
export async function verifyAdminKey(key: string): Promise<{ ok: true } | { ok: false; message: string }> {
  const trimmed = key.trim()
  if (!trimmed) {
    return { ok: false, message: "Admin key is required" }
  }
  ...
  if (res.status === 401 || res.status === 403) {
    return { ok: false, message: "Incorrect admin key" }
  }
  return { ok: false, message: `Could not verify admin key (${res.status})` }
}

async function parseJson<T>(res: Response, context: string): Promise<T> {
  if (res.status === 401 || res.status === 403) {
    throw new AdminAuthError("Admin key required", res.status)
  }
