  ...
  return (
    <div ref={rootRef} className="group relative inline-flex">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
      >
        <span className="text-sm font-medium text-(--text-primary)">{current.triggerLabel}</span>
        <IconChevronDown
            ...
            <li key={opt.code}>
              ...
              >
                <span className="inline-flex items-center gap-2">
                  <span className="inline-block min-w-5">{opt.triggerLabel}</span>
                  <span>{opt.name}</span>
                </span>
              </button>
            </li>
...
export function NavbarToolbar({ historySlot }: Readonly<{ historySlot: ReactNode }>) {
  return (
