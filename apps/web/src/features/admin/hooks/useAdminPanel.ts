...
export function useAdminPanel() {
  ...
  async function loadOverview(cancelledRef: { cancelled: boolean }) {
    try {
  ...
  async function handleRunBatchEvaluation() {
    ...
    try {
      ...
      async function runOne(row: AdminModelListItem, index: number, total: number) {
        setBatchProgress({
          total,
          completed: index,
          currentRoutingKey: row.routing_key,
        })
        setBatchLogs((prev) => [...prev.slice(-80), `[${index + 1}/${total}] ${row.routing_key} - running ${batchMode}`])
        try {
          ...
          setBatchLogs((prev) => [...prev.slice(-80), `[${index + 1}/${total}] ${row.routing_key} - ${statusKey}${skipSuffix}`])
        } catch (err) {
          if (handleAdminError(err)) throw err
          failed += 1
          const message = `${row.routing_key}: ${(err as Error).message}`
          errorMessages.push(message)
          setBatchLogs((prev) => [...prev.slice(-80), `[${index + 1}/${total}] ${message}`])
        }
      }

      for (let index = 0; index < selected.length; index += 1) {
        const row = selected[index]
        await runOne(row, index, selected.length)
  ...
  return {
    ...
    reloadOverview: async () => {
      await loadOverview({ cancelled: false })
    },
  }
}
